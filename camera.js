class Camera {
  ...
  #extrudeCar(poly, carInfo, height = 15, wheelRadius = 5) {
    ...
    this.#moveInward(backLeft, backRight, 0.2);
  ...
  #getPolys(world) {
    ...
    for (const poly of buildingPolys) {
      poly.fill = "rgba(150, 150, 150, 0.4)";
      poly.stroke = "rgba(150, 150, 150, 0.4)";
    }

    const polys = [...carShadows, ...buildingPolys, ...roadPolys, ...carPolys];
  ...
  render(ctx, world) {
    ...
    for (let i = 0; i < projPolys.length; i++) {
      ...
      const distance = polys[i].distanceToPoint(this);
